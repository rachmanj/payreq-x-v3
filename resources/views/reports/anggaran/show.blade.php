        ...
        <div class="col-12">
            <div class="card card-info">
                <div class="card-header">
                    ...
                    <a href="{{ route('reports.anggaran.index') }}" class="btn btn-sm btn-primary float-right"><i
                ...
                <div class="card-body">
                    ...
                    @elseif ($spendingWarning)
                        <div class="alert alert-warning">Utilization is at or above the alert threshold
                            ({{ (int) ($anggaran->warning_threshold ?? 80) }}%).</div>
                    @endif
                    <dl class="row">
                        <dt class="col-sm-4">RAB No</dt>
                        <dd class="col-sm-8">: {{ $anggaran->nomor }}
                            {{ $anggaran->rab_no != null ? '| ' . $anggaran->rab_no : '' }} @if ($anggaran->filename)
                                <a href="{{ asset('file_upload/') . '/' . $anggaran->filename }}"
                                    class='btn btn-xs btn-success' target=_blank>Show RAB</a>
                            @endif
                        </dd>
                        <dt class="col-sm-4">Date | Periode</dt>
                        <dd class="col-sm-8">: {{ date('d-M-Y', strtotime($anggaran->date)) }} |
                            {{ $anggaran->periode_anggaran !== null ? date('M-Y', strtotime($anggaran->periode_anggaran)) : '-' }}
                        </dd>
                        ...
                        <dd class="col-sm-8">:
                            {{ $anggaran->periode_anggaran != null ? date('M Y', strtotime($anggaran->periode_anggaran)) : '-' }}
                            | {{ $anggaran->periode_ofr != null ? date('M Y', strtotime($anggaran->periode_ofr)) : '-' }}
                        </dd>

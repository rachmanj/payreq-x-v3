      ...
      <div class="card-header">
        ...
        <div class="input-group">
          ...
          @foreach($accounts as $account)
            <option value="{{ $account->id }}" {{ $account->id == $selected_account->id ? 'selected' : '' }} >{{ $account->account_number . ' - ' . $account->account_name }}</option>
          @endforeach
          </select>
...
<script>
  $(function () {
    $("#general-ledgers").DataTable({
      processing: true,
      serverSide: true,
      ajax: '{{ route('general-ledgers.data', $selected_account->id) }}',
      columns: [

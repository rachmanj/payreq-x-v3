...
<div class="wrapper mx-4">
  ...
  <section class="invoice">
    ...
    <div class="row invoice-info">
      ...
      <div class="col-sm-4 text-center"><h3>Realization Advance Payment</h3></div>
      <div class="col-sm-4 invoice-col">
        Document No : <b>{{ $realization->nomor }}</b><br>
        Approved Date : <b>{{ $approved_at->format('d M Y') }}</b><br>
        Payreq No : <b>{{ $realization->payreq->nomor }} | {{ \Carbon\Carbon::parse($realization->payreq->approved_at)->addHours(8)->format('d M Y') }}</b><br>
      </div>
    </div>
        ...
        <table class="table table-bordered" style="border: 1px solid black;">
          ...
          <tbody>
            ...
            @endif
              ...
              <td class="text-right" style="border: 1px solid black;">{{ number_format($item->amount, 2) }}</td>
          </tr>
            @endforeach
          </tbody>
          <tfoot>
          ...
          <th class="text-right" style="border: 1px solid black;">{{ number_format($realization_details->sum('amount'), 2) }}</th>
            </tr>
            <tr>
              <th colspan="2" class="text-right" style="border: 1px solid black;">Advance Payment</th>
          <th class="text-right" style="border: 1px solid black;">{{ number_format($realization->payreq->amount, 2) }}</th>
            </tr>
            <tr>
              <th colspan="2" class="text-right" style="border: 1px solid black;">Variance</th>
          <th class="text-right" style="border: 1px solid black;">{{ number_format($realization->payreq->amount - $realization_details->sum('amount'), 2) }}</th>
            </tr>
            <tr>
          ...
          <td colspan="2" style="border: 1px solid black;">{{ ucfirst($terbilang) }}</td>
            ...
            <tr>
              ...
              <td colspan="2" style="border: 1px solid black;">{{ $realization->payreq->remarks }}</td>
            </tr>
          </tfoot>
        </table>

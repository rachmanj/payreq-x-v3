...
@section('title_page', 'Bilyet History')

@section('content')
    <x-bilyet-links page="history" />
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">
                            <i class="fas fa-history"></i> History for {{ $bilyet->full_nomor }}
                        </h3>
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-6">
                                <div class="info-box">
                                    ...
                                    <div class="info-box-content">
                                        <span class="info-box-text">Bilyet Number</span>
                                        <span class="info-box-number">{{ $bilyet->full_nomor }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="info-box">
                                    ...
                                    <div class="info-box-content">
                                        ...
                                        <span class="info-box-number">{{ $bilyet->giro->bank->name ?? 'N/A' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="alert alert-info">
                            ...
                            <p class="mb-0">
                                ...
                                <span
                                    class="badge badge-{{ $bilyet->status == 'onhand' ? 'primary' : ($bilyet->status == 'release' ? 'warning' : ($bilyet->status == 'cair' ? 'success' : 'danger')) }}">
                                    {{ $bilyet->status_label }}
                                </span>
                                <br>
                                <strong>Amount:</strong> {{ number_format($bilyet->amount, 2) }}
                                <br>
                                <strong>Type:</strong> {{ $bilyet->type_label }}
                                <br>
                                <strong>Created:</strong> {{ $bilyet->created_at->format('d M Y H:i:s') }} by
                        ...
                        <div class="timeline">
                            @foreach ($audits as $audit)
                                <div class="time-label">
                                    <span
                                        class="bg-{{ $audit->action == 'created' ? 'green' : ($audit->action == 'voided' ? 'red' : 'blue') }}">
                                ...
                                <div>
                                    <i
                                        class="fas fa-{{ $audit->action == 'created' ? 'plus' : ($audit->action == 'voided' ? 'times' : 'edit') }} bg-{{ $audit->action == 'created' ? 'green' : ($audit->action == 'voided' ? 'red' : 'blue') }}"></i>
                        ...
                        @if ($audits->isEmpty())
                            <div class="alert alert-warning">
                                <i class="fas fa-exclamation-triangle"></i> No history records found for this bilyet.
                            </div>
                        @endif
                    </div>
                </div>

        ...
        <div class="col-12">
            <div class="card card-outline card-primary">
                <div class="card-header">
                    <h3 class="card-title">Create Bank Transaction</h3>
                </div>
                <div class="card-body">

                    <form action="{{ route('cashier.bank-transactions.store') }}" method="POST" id="transaction-form">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('project') is-invalid @enderror"
                                        id="project_display" value="{{ Auth::user()->project }}" readonly>
                                    <input type="hidden" name="project" id="project" value="{{ Auth::user()->project }}">
                                    @error('project')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" name="bank_account" id="bank_account"
                                        value="{{ is_array(old('bank_account')) ? old('bank_account')[0] : old('bank_account') }}">
                                    @error('bank_account')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('transaction_type') is-invalid @enderror"
                                        ...
                                        <option value="transfer_to_petty_cash"
                                            {{ old('transaction_type') === 'transfer_to_petty_cash' ? 'selected' : '' }}>
                                            Transfer to Petty Cash</option>
                                        <option value="bank_admin_fee"
                                            {{ old('transaction_type') === 'bank_admin_fee' ? 'selected' : '' }}>Bank Admin
                                            Fee</option>
                                        <option value="bank_interest"
                                            {{ old('transaction_type') === 'bank_interest' ? 'selected' : '' }}>Bank
                                            Interest</option>
                                    </select>
                        ...
                        <div class="mt-4">
                            <button type="submit" class="btn btn-primary btn-sm" id="save-transaction-btn">Save
                                Transaction</button>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary btn-sm" id="save-detail-btn">Add Detail</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#add-detail-btn').on('click', function() {
                ...
                // Clear select2 and repopulate for current transaction type
            ...
            $('#save-detail-btn').on('click', function() {
                ...
                // Add row to table
                addDetailRow(realizationDate, accountCode, accountName, debitCredit, description, project,
                    costCenter, amount);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        const oldBankAccount =
                            "{{ is_array(old('bank_account')) ? old('bank_account')[0] : old('bank_account') }}";
                        console.log('User project:', userProject);
                        console.log('Old bank account:', oldBankAccount);
                        ...
                        if (response && response.length > 0) {
                            ...
                            response.forEach(function(account) {
                                ...
                                const selected = oldBankAccount === accountNumber ? 'selected' :
                                    '';

                                bankAccountSelect.append(
                                    `<option value="${accountNumber}" ${selected}>${accountNumber} - ${accountName}</option>`
                                );
                            });

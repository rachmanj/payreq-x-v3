            ...
            <div class="card card-info">
                <div class="card-header">
                    ...
                    <a href="{{ route('accounting.sap-sync.index', ['page' => $vj->project]) }}"
                        class="btn btn-sm btn-primary float-right"><i class="fas fa-arrow-left"></i> Back</a>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-8">
                            <div class="row">
                                ...
                                <dd class="col-sm-8">: {{ $vj->sap_journal_no }} @if ($vj->sap_filename)
                                        <a href="{{ asset('file_upload/') . '/' . $vj->sap_filename }}"
                                            class='btn btn-xs btn-success' target=_blank>SAP Journal</a>
                                    @endif
                                </dd>
                                ...
                                <dd class="col-sm-8">: {{ $vj->createdBy->name }} on
                                    {{ date('d-M-Y H:m', strtotime($vj->created_at . '+8 hours')) }} wita</dd>
                                <dt class="col-sm-4">Posted by</dt>
                                <dd class="col-sm-8">:
                                    {{ $vj->posted_by ? $vj->postedBy->name . ' on ' . date('d-M-Y H:m', strtotime($vj->updated_at . '+8 hours')) . ' wita' : 'not posted yet' }}
                                </dd>
                            </div>
                        </div>
                        <div class="col-4">
                            @if ($vj->sap_journal_no)
                                <button class="btn btn-outline-danger btn-lg"
                                    style="pointer-events: none;"><b>POSTED</b></button>
                            @endif
                        </div>
                    </div>
                ...
                <div class="card-header">
                    ...
                    <form action="{{ route('accounting.sap-sync.cancel_sap_info') }}" method="POST">
                        ...
                        <button class="btn btn-xs btn-danger float-right" {{ $vj->sap_journal_no ? '' : 'disabled' }}
                            onclick="return confirm('Are You sure You want to cancel this SAP Info? This action cannot be undone')">Cancel
                            SAP Info</button>
                    </form>
                    <button class="btn btn-xs btn-warning float-right mr-2" data-toggle="modal" data-target="#update-sap"
                        style="color: black; font-weight: bold" {{ $vj->sap_journal_no ? 'disabled' : '' }}>Update SAP
                        Info</button>
                    <button class="btn btn-xs btn-warning float-right mr-2" data-toggle="modal"
                        data-target="#upload-journal" style="color: black; font-weight: bold"
                        {{ $vj->sap_journal_no ? '' : 'disabled' }}>Upload SAP Journal</button>
                    <a href="{{ route('accounting.sap-sync.export', ['vj_id' => $vj->id]) }}"
                        class="btn btn-xs btn-warning float-right mr-2" style="color: black; font-weight: bold">Export to
                        Excel</a>
                    <a href="{{ route('verifications.journal.print', $vj->id) }}"
                        class="btn btn-xs float-right mr-2 btn-warning" style="color: black; font-weight: bold"
                        target="_blank">Print Journal</a>
                    @if ($vj->sap_journal_no === null)
                        <a href="{{ route('accounting.sap-sync.edit_vjdetail_display', ['vj_id' => $vj->id]) }}"
                            class="btn btn-xs btn-warning float-right mr-2" style="color: black; font-weight: bold">Edit
                            VJDetails</a>
                    @endif
                </div>
                <div class="card-body">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if ($item['account_name'] === 'not found')
                                            <span
                                                style="color: red;"><small><b>{{ $item['account_name'] }}</b></small></span>
                                        @else
                            ...
                            <tr>
                                ...
                                <th class="text-right">
                                    {{ number_format($vj_details->where('debit_credit', 'debit')->sum('amount'), 2) }}</th>
                                <th class="text-right">

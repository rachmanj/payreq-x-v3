    ...
    <div class="vj-show">
                    ...
                    <div class="card-header bg-gradient-{{ $headerGradient }} text-white">
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h3 class="card-title mb-0 text-white">Manual Journal Entry</h3>
                                    <small class="text-white-50">{{ $journalEntry->number }}</small>
                                </div>
                            </div>
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                <div class="card card-outline card-primary">
                    <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
                        <h3 class="card-title mb-0"><i class="fas fa-info-circle"></i> Header</h3>
                    </div>
                    <div class="card-body">
                        <dl class="row mb-0">
            ...
            <div class="col-md-6">
                <div class="card card-outline card-primary">
                    <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
                        <h3 class="card-title mb-0"><i class="fas fa-cloud-upload-alt"></i> SAP Integration</h3>
                    </div>
                    <div class="card-body">
                        <dl class="row mb-0">
                            ...
                            <dd class="col-sm-7">
                                @if ($journalEntry->isReversed())
                                    <span class="vj-chip vj-chip-neutral">Reversed</span>
                                @elseif ($journalEntry->isPosted())
                                    <span class="vj-chip vj-chip-success">Posted</span>
                                @elseif ($journalEntry->sap_submission_status === 'failed')
                                    <span class="vj-chip vj-chip-danger">Failed</span>
                                @else
                                    <span class="vj-chip vj-chip-warning">Draft</span>
                                @endif
                            </dd>
                        ...
                        <div class="vj-inline-actions mt-3">
                            @if ($journalEntry->isEditable())
                                <a href="{{ route('accounting.journal-entries.edit', $journalEntry->id) }}" class="vj-action-item vj-action-edit">
                                    <i class="fas fa-edit"></i>
                                    <span>Edit</span>
                                </a>
                                <form action="{{ route('accounting.journal-entries.submit_to_sap', $journalEntry->id) }}" method="POST" class="vj-action-item-form d-inline"
                                    onsubmit="return confirm('Submit this journal entry to SAP B1?');">
                                    @csrf
                                    <button type="submit" class="vj-action-item vj-action-item-btn vj-action-sap">
                                        <i class="fas fa-paper-plane"></i>
                                        <span>Submit to SAP B1</span>
                                    </button>
                                </form>
                            @endif

                            @if ($journalEntry->isPosted() && ! $journalEntry->isReversed() && auth()->user()->can('cancel_sap_journal'))
                                <button type="button" class="vj-action-item vj-action-item-btn vj-action-cancel" data-toggle="modal" data-target="#reverse-modal">
                                    <i class="fas fa-undo"></i>
                                    <span>Reverse in SAP B1</span>
                                </button>
                            @endif

                            <a href="{{ route('accounting.journal-entries.print', $journalEntry->id) }}" class="vj-action-item vj-action-print" target="_blank">
        ...
        <div class="row mb-3">
            <div class="col-12">
                <div class="card card-outline card-primary">
                    <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
                        <h3 class="card-title mb-0"><i class="fas fa-list"></i> Journal Lines</h3>
                    </div>
                    <div class="card-body table-responsive p-0">
                        <table id="je-show-lines-table" class="table table-bordered table-sm mb-0">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Account</th>
        ...
        @if ($submissionLogs->isNotEmpty())
            <div class="row">
                <div class="col-12">
                    <div class="card card-outline card-primary">
                        <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
                            <h3 class="card-title mb-0"><i class="fas fa-history"></i> Submission History</h3>
                        </div>
                        <div class="card-body p-0">
                            <table id="je-submission-logs-table" class="table table-sm mb-0">
                                <thead>
                                    <tr>
                                        <th>Date</th>
                                        <th>Action</th>
                                ...
                                <tbody>
                                    @foreach ($submissionLogs as $log)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="vj-chip vj-chip-{{ $log->status === 'success' ? 'success' : 'danger' }}">
                                                    {{ ucfirst($log->status) }}
                                                </span>
                                            </td>
                                            <td>{{ $log->user?->name }}</td>
                    ...
                    <div class="modal-content">
                        <form action="{{ route('accounting.journal-entries.reverse_to_sap', $journalEntry->id) }}" method="POST">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="vj-action-item vj-action-back" data-dismiss="modal">Cancel</button>
                                <button type="submit" class="vj-btn vj-btn-danger">Reverse in SAP B1</button>
                            </div>
                        </form>
                    </div>
                </div>

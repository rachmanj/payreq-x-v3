...
class Anggaran extends Model
{
    ...
    public const FUND_STATUS_PENDING = 'pending';

    public const FUND_STATUS_POOLED = 'pooled';

    public const FUND_STATUS_RELEASED = 'released';

    protected $guarded = [];

    protected $casts = [
        'fund_pooled_at' => 'datetime',
    ];

    public function payreqs()
    {
        return $this->hasMany(Payreq::class, 'rab_id');
    }

    public function details()
    {
        return $this->hasMany(AnggaranDetail::class)->orderBy('sort_order')->orderBy('id');
    }
    ...
    public function fundPooledBy()
    {
        return $this->belongsTo(User::class, 'fund_pooled_by');
    }
